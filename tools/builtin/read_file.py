...
class ReadFileParams(BaseModel):
    ...
    offset: int = Field(
        1, 
        ge=1, 
        description="Line number to start reading from (1-based index). Default is 1",
        )
...
class ReadFileTool(Tool):
    name: str = "read_file"
    description= (
        "Reads the contents of a text file. Returns the file content with line number. "
        "For large files, use offset and limit to read sepcific portions. "
        ...
        try:
            ...
            if token_count > self.MAX_OUTPUT_TOKENS:
                output = truncate_text(
                    ...
                    suffix=f"\n... [truncated {total_lines} totals lines]",
                )
                truncated = True
